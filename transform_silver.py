...
# Motivo
support_motivo_path = os.listdir("data/bronze/support/motivos/")[0]
df_mot = pl.read_csv(
    f"data/raw/support/motivos/{support_motivo_path}",
    separator=";",
...
# Municipio
support_municipio_path = os.listdir("data/bronze/support/municipios/")[0]
df_mun = pl.read_csv(
    f"data/raw/support/municipios/{support_municipio_path}",
    separator=";",
...
# Pais
support_pais_path = os.listdir("data/bronze/support/paises/")[0]
df_paises = pl.read_csv(
    f"data/raw/support/paises/{support_pais_path}",
    separator=";",
    encoding="latin1",
...
for index, file in enumerate(estabelecimentos_list):
    if file.endswith(".parquet"):
        ...
        ).write_parquet(f"data/gold/matriz/estabelecimento_matriz_{index}.parquet")

        print(f"Dumping filial,file: {index}")
        # Dump to parquet for filial
        df_emp.select(select_columns).filter(
            pl.col("identificador_matriz") == "filial"
        ).write_parquet(f"data/gold/filial/estabelecimento_filial_{index}.parquet")
